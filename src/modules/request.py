    ...
    for j in range(12):
        ...
        if r.status_code == 200:
            ...
            try:
                ...
                insert['{}_avg_max_tempF'.format(month[j])] = np.nan
                insert['{}_avg_min_tempF'.format(month[j])] = np.nan
                insert['{}_precipIn'.format(month[j])] = np.nan
                insert['{}_humidPerc'.format(month[j])] = np.nan
